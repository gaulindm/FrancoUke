...
def select_variations(base_name, all_variations, user_pref_show_alternates, requested_variations):
    """
    all_variations: list of dicts (v0, v1, v2...)
    requested_variations: dict { base_name -> forced_index }

    RULES:
    - Always include v0
    - If song forces variation N (via [C(N)]), include variation N even if user disables alternates
...
def parse_requested_variation(chord_name: str):
    ...
    if not match:
        return chord_name, None  # fallback
...
def draw_footer(
    ...
    # -----------------------------------------------------
    # PREPARE PRIMARY INSTRUMENT DIAGRAMS
    # -----------------------------------------------------
    ...
    def draw_diagrams(diagrams: List[Dict[str, Any]], start_x: float, start_y: float, inst: str):
        ...
        for row in rows:
            # center diagrams inside page width
    ...
    # -----------------------------------------------------
    # DRAW PRIMARY DIAGRAMS
    # -----------------------------------------------------
    draw_diagrams(primary_diagrams, 0, start_y, instrument)
